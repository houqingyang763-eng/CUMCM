...
示例：py -3.13 experiments/q4/run.py --split smoke --policies joint25 baseline
      --output outputs/experiments/q4/smoke01 --workers 2
...
def source_paths():
    ...
    paths = {ROOT / name for name in ("run.py", "analyze.py", "test_runner.py", "planner.py")}
    for module in tuple(sys.modules.values()):
        ...
        if path.suffix == ".py" and PROJECT in path.parents:
            paths.add(path)
    # q2_geometry通过动态加载使用几何底座；也记录用于移植等价性审计的源文件。
    paths.add(PROJECT / "experiments/b_adaptive_q3/geometry.py")
    paths.add(PROJECT / "experiments/b_overnight/q4_anchor_design.py")
    return sorted(paths)
...
def main():
    ...
    parser.add_argument("--policies", nargs="+", default=["baseline", "joint25"])
    ...
    output_root = (PROJECT / "outputs/experiments/q4").resolve()
    if output_root not in output.parents:
        raise ValueError("结果必须放outputs/experiments/q4/<run-id>/子目录")

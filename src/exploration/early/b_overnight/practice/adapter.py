...
def strategy(problem, name=None, overrides=None):
    ...
    def validate(value):
        ...
        if isinstance(value, (list, tuple)):
            for x in value:
                validate(x)
    validate(overrides)
    if problem == 3 and name in ("completion", "configured", "sweep"):
        ...
        if name == "sweep":
            ...
            return Q3SweepPolicy(config), InformationState()
    ...
    raise ValueError("问题与策略不匹配；只支持 Q3 completion/configured/sweep 和 Q4 joint25/directional")

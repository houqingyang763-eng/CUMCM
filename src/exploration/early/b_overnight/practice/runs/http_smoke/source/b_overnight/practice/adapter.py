...
def public_response(response):
    ...
    fields = ("accepted", "real_timestamp_ms", "virtual_time_s", "remaining_real_duration_s",
              "max_real_duration_s", "max_virtual_duration_s", "measure_result", "svd_deg",
              "clear_result", "exit_reason")
...
def strategy(problem, name, overrides=None):
    ...
    raise ValueError("问题与策略不匹配；只支持 Q3 completion/configured 和 Q4 directional")
...
def run_session(client, policy, state, problem, authorization, output, guard=check_cached, audit_hook=None, reserve_real_s=10.0):
    ...
    summary = dict(scope=authorization.scope, problem=problem, policy=type(policy).__name__,

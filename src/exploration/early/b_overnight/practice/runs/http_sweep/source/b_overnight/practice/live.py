...
def main():
    ...
    parser.add_argument("--strategy", choices=("completion", "configured", "sweep", "directional", "joint25"),
                        help="省略时问题3用completion，问题4用joint25")
    ...
    strategy_name = args.strategy or {3: "completion", 4: "joint25"}[args.problem]
    bundle = capture_sources()
    policy, state = strategy(args.problem, strategy_name, overrides)
    authorization = load_verified_practice_ui(args.ui_receipt, args.problem, args.port)
    ...
    save_sources(args.output, bundle)
    unchanged = sources_unchanged(bundle)
    manifest = {"created_at": time.time(), "strategy": strategy_name, "problem": args.problem,
                "source_sha256": source_hashes(bundle), "sources_unchanged_during_session": unchanged,
                "config_sha256": hashlib.sha256((args.output / "config.json").read_bytes()).hexdigest(),
                "official_ui_verification": authorization.evidence_record,
                "official_result_still_requires_ui_review": True}
    dump(args.output / "manifest.json", manifest)
    print(json.dumps(result, ensure_ascii=False, indent=2))
    if result["error_type"] or not result["complete_by_evidence"] or not unchanged:
        raise SystemExit(1)

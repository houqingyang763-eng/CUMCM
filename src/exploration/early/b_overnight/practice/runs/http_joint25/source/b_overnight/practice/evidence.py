"""实际演练 UI 的模式核验收据；它记录证据，不要求新的用户许可。"""
...
class ReviewedPracticeAuthorization:
    ...
    def __init__(self, receipt_path, problem, port):
        self.receipt_path = Path(receipt_path).resolve()
        self.problem, self.port = problem, port
    ...
    def authorize(self, port, problem):
        ...
        self.evidence_record = {key: data[key] for key in ("observed_at_utc", "problem", "case_code", "mode_visible_text", "window_title", "screenshot_sha256", "review_note")}
...
def load_verified_practice_ui(receipt_path, problem, port):
    # 图像语义须由实际操作者（人或已获授权代理）核对；代码只核验新鲜度和完整性。
    validate_receipt(receipt_path, problem, port)
    return ReviewedPracticeAuthorization(receipt_path, problem, port)

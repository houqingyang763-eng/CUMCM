...
from q4_search import (Q4SearchConfig, Q4SearchPolicy, Q4CoverageInformationState,
                       ...
                       interval_direction, Q4HullSearchPolicy, box_remaining_probes, hull_gain)
...
class SearchTests(unittest.TestCase):
    ...
    def test_box_work_stages_reach_zero_only_after_full_surround(self):
        box = Box(-10, -10, 10, 10)
        self.assertEqual(box_remaining_probes(box, ()), 3)
        self.assertEqual(box_remaining_probes(box, ((500, 500),)), 2)
        self.assertEqual(box_remaining_probes(box, ((500, 500), (-500, 500))), 1)
        self.assertEqual(box_remaining_probes(box, ((500, 500), (-500, 500), (0, -500))), 0)

    def test_very_thin_angular_gap_still_has_one_probe_of_work(self):
        box = Box(-0.01, -0.01, 0.01, 0.01)
        near = (g.mul(g.unit(89.99), 900), g.mul(g.unit(-89.99), 900))
        self.assertEqual(box_remaining_probes(box, near), 1)
        self.assertEqual(box_remaining_probes(box, near + ((-900, 0),)), 0)

    def test_hull_repeated_measure_has_zero_gain(self):
        self.assertGreater(hull_gain((), (0, 0)), 0)
        self.assertEqual(hull_gain(((0.0, 0.0),), (0, 0)), 0)


def batch(output, seed, variant="angular"):
    ...
    results = []
    candidate_name, candidate_cls = ("search", Q4SearchPolicy) if variant == "angular" else ("hull_search", Q4HullSearchPolicy)
    for scene in scenes:
        for name, policy_cls, state_cls in (("directional", Q4DirectionalPolicy, Q4InformationState),
                                            (candidate_name, candidate_cls, Q4CoverageInformationState)):
            check_cached()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--variant", choices=("angular", "hull"), default="angular")
    args, rest = parser.parse_known_args()
    if args.batch:
        batch(args.batch, args.seed, args.variant)
    else:

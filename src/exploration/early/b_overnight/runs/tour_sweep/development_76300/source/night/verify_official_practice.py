...
if __name__ == "__main__":
    check_cached()
    cases = ["q3_completion_20260911", "q4_joint25_20260911"]
    records = [verify(ROOT / "practice" / "local_runs" / name) for name in cases]
    out = ROOT / "runs" / "official_practice"
    out.mkdir(exist_ok=True)
    result = dict(cases=records, interpretation="两次不同问题的单局真实官方演练。源数由实际结束UI核对，全清由不同频道成功回执与该源数共同验证；不是正式成绩，也不能作跨策略配对比较。")

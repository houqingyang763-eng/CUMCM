...
from task_cost import CompletionCostConfig, CompletionCostPolicy, TaskCostConfig, TaskCostPolicy, g
...
def smoke(output, variant="task_cost"):
    import run_local
    ...
    policy_class, policy_config = ((CompletionCostPolicy, CompletionCostConfig()) if variant == "completion_cost"
                                   else (TaskCostPolicy, TaskCostConfig()))
    run_local.write_json(output / "config.json", policy_config.to_dict())
    ...
    results = []
    for case in cases:
        for label, cls, config in [("baseline", BasePolicy, Config()), (variant, policy_class, policy_config)]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--variant", choices=("task_cost", "completion_cost"), default="task_cost")
    args, rest = parser.parse_known_args()
    if args.smoke:
        smoke(args.smoke, args.variant)
    else:

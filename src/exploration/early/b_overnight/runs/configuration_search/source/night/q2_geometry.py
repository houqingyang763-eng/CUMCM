...
def wedge_with_error(poly, position, bearing, error_deg=g.ANGLE_ERROR):
    ...
    return g.clip(g.clip(poly, nlo, g.dot(nlo, position)),
                  nhi, g.dot(nhi, position))


def bearing_halfplanes(position, bearing, error_deg=1.0):
    lo, hi = g.unit(bearing - error_deg), g.unit(bearing + error_deg)
    return [(n, g.dot(n, position)) for n in ((lo[1], -lo[0]), (-hi[1], hi[0]))]
...
def first_region(position, bearing, sides=360, error_deg=g.ANGLE_ERROR):
    """首次 direction 反馈的凸外包；保留 5 米近场孔，属于保守外包。"""
...
def run_demo(output, bin_width_deg=1.0):
    ...
    # 同时保留纯几何候选和移动成本折中；后者不是完成清除的时间估计。
    rows.sort(key=lambda row: row["worst_radius_upper_m"])
    for row in rows:
        row["geometry_plus_move_proxy_m"] = row["worst_radius_upper_m"] + 0.02 * g.distance(p, row["q"])
    best = rows[0]
    balanced = min(rows, key=lambda row: row["geometry_plus_move_proxy_m"])
    fine = []
    for q in dict.fromkeys([tuple(best["q"]), tuple(balanced["q"]), (750.0, 600.0), (500.0, 800.0)]):
        _guard()
    ...
    result = {"schema": "q1_q2_geometry_v1", "units": "m,degree,s",
              "input": {"first_position": p, "first_bearing_deg": 0.0,
                        ...
                        "candidate_inner_sides": 128, "movement_penalty_m_per_m": 0.02},
              "source_outer_polygon": poly, "C1000_inner_polygon": c1000,
              "candidates": rows, "best_geometry": best, "balanced_proxy": balanced,
              "fine_bounds": fine,
              ...
              "source_sha256": {str(path.relative_to(Path(__file__).resolve().parents[2])):
                  ...
                  for path in (Path(__file__), BASE / "geometry.py")},
    ...
    print(json.dumps({"candidate_count": len(rows), "best_q": best["q"],
                      ...
                      "balanced_q": balanced["q"], "fine": fine,

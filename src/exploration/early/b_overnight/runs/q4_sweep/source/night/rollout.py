...
class RolloutPolicy:
    ...
    def make_base(self):
        ...
        if self.config.continuation_kind == "completion":
            from task_cost import CompletionCostPolicy, CompletionCostConfig
            cls, config = CompletionCostPolicy, CompletionCostConfig()
    ...
    def choose(self, state):
        ...
        try:
            ...
            if any(e is None for e in environments):
                ...
                return original
            # 按共同环境逐轮比较，预算不足时保留已完整评估的共同轮次。
            # 不能拿某候选两局均值与另一候选一局均值相比。
            ...
            for env in environments:
                ...
                if len(round_costs) != len(candidates):
                    break
                for column, cost in zip(columns, round_costs):
                    column.append(cost)
                completed_worlds += 1
            if not completed_worlds:
                return original  # 不把被截断的后续任务当免费。
            values = [statistics.mean(costs)+self.config.tail_weight*max(costs) for costs in columns]
            ...
            if winner and values[winner] < values[0]*(1-self.config.min_relative_gain):
                ...
                action["compared_worlds"] = completed_worlds
                return action
            return original
        finally:

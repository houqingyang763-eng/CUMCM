...
def intersect_intervals(first, second):
    # 0 与360为同一闭端点；补齐两种表示后再在线段上求交。
    first, second = list(first), list(second)
    if any(a == 0 or b == 360 for a, b in first):
        first += [(0.0, 0.0), (360.0, 360.0)]
    if any(a == 0 or b == 360 for a, b in second):
        second += [(0.0, 0.0), (360.0, 360.0)]
...
class Q4JointCoverageInformationState(Q4CoverageInformationState):
    """保留未知频道的连续覆盖证书，同时加强已发现频道的联合清除格。

    参数及 discovery_points/coverage/update/absence_proofs 完全继承覆盖
    状态，可直接作为精简锚点策略的 q4run.run_case(state_factory=...)。
    """
    def __init__(self, coverage_depth=7, coverage_refinement_depth=10):
        super().__init__(coverage_depth, coverage_refinement_depth)
        self.channels = {j: JointChannelState(j) for j in range(1, 21)}

    joint_statistics = Q4JointInformationState.joint_statistics

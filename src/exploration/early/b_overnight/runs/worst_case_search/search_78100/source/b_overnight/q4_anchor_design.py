...
def symmetric25_evidence(output):
    check_cached()
    output.mkdir(parents=True, exist_ok=False)
    a, points = Q4_SYMMETRIC25_SPACING_M, Q4_SYMMETRIC25_ANCHORS
    cert = q4_coverage_certificate(points)
    verify_q4_certificate(cert, require_complete=True)
    assert len(points) == 25 and len(g.hull(list(points))) == 12
    triangles = [triple for triple in combinations(range(19), 3)
                 if all(abs(g.distance(points[i], points[j]) - a) < 1e-5 for i, j in combinations(triple, 2))]
    assert len(triangles) == 24
    for k in range(6):
        desired = [g.mul(g.unit(60 * k), 2 * a),
                   g.mul(g.unit(30 + 60 * k), math.sqrt(3) * a),
                   g.mul(g.unit(60 + 60 * k), 2 * a)]
        indices = [min(range(19), key=lambda i: g.distance(points[i], q)) for q in desired]
        assert all(g.distance(points[i], q) < 1e-5 for i, q in zip(indices, desired))
        triangles.extend(((indices[0], indices[1], 19 + k), (indices[1], indices[2], 19 + k)))
    edges = [g.distance(points[i], points[j]) for tri in triangles for i, j in combinations(tri, 2)]
    area = sum(g.area([points[i] for i in tri]) for tri in triangles)
    assert len(triangles) == 36 and max(edges) < 1000
    assert abs(area - g.area(g.hull(list(points)))) < 1e-5
    inradius = 2 * a * math.cos(math.pi / 12)
    max_edge = 4 * a * math.sin(math.pi / 12)
    assert inradius > 1800 and max_edge < 1000
    dump(output / "symmetric25_geometry.json", dict(
        point_count=25, points=points, spacing_m=a, outer_radius_m=2 * a,
        polygon_inradius_m=inradius, source_domain_margin_m=inradius - 1800,
        maximal_triangle_edge_m=max_edge, reception_margin_m=1000 - max_edge,
        triangulation=triangles, triangulation_area_m2=area,
        certificate=cert.to_dict(include_leaves=True)))
    print(json.dumps(dict(points=25, triangles=36, polygon_inradius_m=inradius,
                           maximal_triangle_edge_m=max_edge, certificate=cert.statistics)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--symmetric25", type=Path)
    parser.add_argument("--seed", type=int)
    args = parser.parse_args()
    target = args.design or args.batch or args.scales or args.scaled_batch or args.symmetric25
    ...
    elif args.symmetric25:
        symmetric25_evidence(output)
    elif args.scales:

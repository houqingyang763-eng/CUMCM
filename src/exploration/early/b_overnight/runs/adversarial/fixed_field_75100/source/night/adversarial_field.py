    ...
    def act(self, action):
        ...
        if key not in self.entries:
            ...
            if result == "direction":
                ...
                best = max(alternatives, key=lambda a: (a["area_m2"], a["diameter_m"],
...
def run_case(scene, problem, out, replay_entries=None):
    check_cached()
    mode = "replay" if replay_entries is not None else "generation"
    policy = CompletionCostPolicy() if problem == "q3" else Q4Symmetric25Policy()
    state = InformationState() if problem == "q3" else Q4JointCoverageInformationState()
    state.action_history = []
    env = FixedHardFieldEnvironment(scene, replay_entries)
    ...
    result = dict(case=scene.name, problem=problem, mode=mode, layout=scene.layout, seed=scene.seed,
                  policy="completion" if problem == "q3" else "symmetric25_joint_coverage",
...
def self_test():
    ...
    for entry in env.entries.values():
        if entry["alternatives"]:
            best = max(entry["alternatives"], key=lambda a: (a["area_m2"], a["diameter_m"],
                                                           abs(a["error_deg"]), a["error_deg"]))
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "runs/adversarial/fixed_field_75100")

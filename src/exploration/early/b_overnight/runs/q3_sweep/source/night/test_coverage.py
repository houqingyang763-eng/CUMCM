...
class CoverageTests(unittest.TestCase):
    ...
    def test_mixin_can_close_small_hole_without_seven_anchors(self):
        ...
        for j in range(2, 21):
            state.channels[j].status = "absent"
        for q in [(999.0, 0)] + ring_points(rotation=17):
            negative_update(state, q)
        ...
        self.assertLess(len(state.channels[1].anchors_missed), len(g.ANCHORS))
...
def save_run_proofs(output):
    """从已有覆盖策略流水重建停止证据，不重新生成案例或运行新策略。"""
    records = []
    for trace in sorted((output / "actions").glob("*_coverage.jsonl")):
        state = CoverageInformationState()
        for line in trace.read_text(encoding="utf-8").splitlines():
            ...
            for j, c in state.channels.items():
                if c.status == "absent" and previous[j] != "absent":
                    record = dict(case=trace.stem, step=row["step"], channel=j,
                                  reason=state.absence_reasons[j],
                                  anchors_missed=sorted(c.anchors_missed))
                    if record["reason"] == "negative_disk_union":
                        cert = state.coverage(j)
                        verify_certificate(cert, require_complete=True)
                        record["certificate"] = cert.to_dict(include_leaves=True)
                    records.append(record)
        assert state.complete
    (output / "absence_proofs.json").write_text(
        json.dumps(records, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(dict(proofs=len(records),
                          before_seven_anchors=sum(len(r["anchors_missed"]) < 7 for r in records),
                          certified_union=sum(r["reason"] == "negative_disk_union" for r in records))))

...
def factories(name, overrides=None):
    ...
    if name in ("task_cost", "coverage_cost"):
        from task_cost import TaskCostConfig, TaskCostPolicy
        policy_cls, state_cls = TaskCostPolicy, InformationState
        if name == "coverage_cost":
            from coverage import CoverageInformationState, CoveragePolicyMixin
            class Policy(CoveragePolicyMixin, TaskCostPolicy):
                pass
            policy_cls, state_cls = Policy, CoverageInformationState
        return lambda: policy_cls(TaskCostConfig(**overrides)), state_cls

...
def check_cached():
    if (LOCAL / "OFF.json").exists():
        return {"mode": "OFF", "remaining_percent": None,
                "reason": "用户明确解除本次额度锁定，不再巡检或阻塞工作"}
...
def daemon(config):
    ...
    while (LOCAL / "ACTIVE.json").exists() and not (LOCAL / "ARCHIVED.json").exists() and not (LOCAL / "OFF.json").exists():
...
def main():
    ...
    if args.command == "activate":
        if (LOCAL / "OFF.json").exists():
            raise RuntimeError("本轮已由用户彻底关闭；需另行明确部署新保护，不能恢复旧配置")
        if (LOCAL / "STOP.json").exists():

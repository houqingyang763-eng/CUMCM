...
仓库根目录运行（队号仅从环境变量读取，不放命令参数、源码或日志）：
  py -3.13 src/q4/practice.py --receipt <本次收据.json>
    --output outputs/q4/<演练批次>
策略固定读取selected_config.json，队号读取CUMCM_ROBOT_ID环境变量。
...
import common
from common import PROJECT, ROOT, Q4State

from public_client import RobotClient, ProbeError
CLIENT_PATH = ROOT / "core/public_client.py"
OFFICIAL_OUTPUT_ROOT = (PROJECT / "outputs/q4").resolve()
...
def policy_and_state(name="selected", overrides=None):
    if name == "selected":
        if overrides:
            raise ValueError("selected只读取唯一正式配置，不接受覆盖")
...
def snapshot_sources(output):
    ...
    paths = {Path(__file__).resolve(), CLIENT_PATH, ROOT / "core/geometry.py",
             ROOT / "planner.py", ROOT / "selected.py", ROOT / "selected_config.json"}
...
def run_session(client, policy, state, authorization, output, reserve_real_s=15., max_actions=5000, audit_hook=None):
    ...
    if authorization.scope == "official_practice" and OFFICIAL_OUTPUT_ROOT not in output.parents:
        raise ValueError("演练产物必须进入outputs/q4/<run-id>")

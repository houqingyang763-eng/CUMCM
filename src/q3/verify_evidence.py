...
OUTPUT = ROOT / "outputs/source_migration/q3/evidence_validation.json"
...
class OrganizationResolver:
    ...
    def __init__(self, manifest=None):
        ...
        for entry in self.manifest["files"]:
            ...
            require(destination.resolve().is_relative_to(ROOT / "src"), f"目标越出 b_q3：{new}")
...
def verify_organization(resolver):
    ...
    return dict(files=len(resolver.entries), bytes=total_bytes, external_dependencies=count,
                unique_old_paths=True, unique_new_paths=True, all_targets_inside_src=True,

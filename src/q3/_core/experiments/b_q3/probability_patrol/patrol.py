...
class PatrolPolicy:
    ...
    def choose_joint(self, state):
        """R3：仅改变途中扫描站与清除任务的共同安排，保留R1局部几何。"""
        ...
        if self.station is not None:
            ...
            if action:
                return action
        if self.after_arrival_scan is not None:
            q, scan_unknown, priority = self.after_arrival_scan
            self.after_arrival_scan = None
            if g.distance(q, state.position) > 1e-6:
                raise AssertionError("计划的到达扫描位置与实际位置不符")
            self.start_station(state, q, "planned_clearance_scan", priority=priority,
        ...
        for c in found:
            center, radius = c.circle()
            can_try = radius <= self.config["try_radius_m"] and c.channel not in self.tried_centers
            q = center if radius <= 20 - 1e-6 or can_try else self.local_point(state, c)
            service_jobs.append({"kind": "service", "position": q, "channel": c.channel})
        route, self.decision_data = covering_route(state, service_jobs)
        self.plans_made += 1
        self.decision_data["plans_made"] = self.plans_made
        if not route:
            raise AssertionError("未结束状态没有覆盖/服务计划")
        job = route[0]
        q = job["position"]
        if job["kind"] == "scan":
            self.target = None
            self.start_station(state, q, "joint_cover_scan", scan_unknown=True)
            action = self.station_action(state)
            if not action:
                raise AssertionError("规划中的未知扫描点没有合法测量")
            return action
        ...
        center, radius = c.circle()
        if radius <= 20 - 1e-6:
            self.after_arrival_scan = (q, job["scan_unknown"], c.channel)
            return self.action("clear", q, c.channel, "certain_center")
        if radius <= self.config["try_radius_m"] and c.channel not in self.tried_centers:
            self.tried_centers.add(c.channel)
            self.after_arrival_scan = (q, job["scan_unknown"], c.channel)
            return self.action("clear", q, c.channel, "single_center_try")
        # 已到达且没测过的视角先利用，测后必须重新规划，不能使用预测反馈。
        if state.position not in c.measured and g.distance(state.position, center) - radius <= 1500:
        ...
        self.start_station(state, q, "planned_transverse_scan", priority=c.channel,
                           scan_unknown=job["scan_unknown"])
        action = self.station_action(state)
        if action:
            return action
        raise AssertionError("规划的服务测点没有合法测量")
    ...
    def choose(self, state):
        ...
        if self.config.get("planning") == "covering":
            return self.choose_joint(state)
